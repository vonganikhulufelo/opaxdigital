  ...
  def crebate
      ...
      if @district.present?
        ...
        if @c_pay_m_product_price1
          ...
          @description2 = Time.now.strftime("%Y-%m-%d %H:%M:%S").to_s + "$" + @c_pay_m_product_price1.product_pescription_product.to_s + "$" + params[:product_price].to_s+ "$" + params[:rebate].to_s + "$" + params[:claw_margin].to_s + "$" + params[:net_price] + "$" + @supplier.supplier_name.to_s + "$" + @supplier_price.net_price.to_s + "$" + @gross_price.to_s
